...
descriptions may be inaccurate or out-of date, please confirm for yourself):
 ...
 - For Web API usage, all of the plugins above provide SDA credentials in the
   ...
   initially obtain an access token (e.g. when you enable my plugin) or if an
   access token expires (e.g. if your system sleeps), then you must authorize
   the plugin again. This entails opening a "Login with Spotify" page in the
   browser and clicking a button. My plugin will open this page automatically
   for you in the browser as-needed, and this behavior is customizable to be
   unobtrusive. However, some users may find it more convenient to just make
   their own SDA and avoid this hassle.
